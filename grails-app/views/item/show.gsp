	...
	<body>
		...
		<div id="show-item" class="content scaffold-show" role="main">
			...
			<ol class="property-list item">
				...
				<g:if test="${itemInstance?.contrapunto}">
				<li class="fieldcontain">
					<span id="contrapunto-label" class="property-label"><g:message code="item.contrapunto.label" default="Contrapunto" /></span>
					
						<span class="property-value" aria-labelledby="contrapunto-label"><g:link controller="contrapunto" action="show" id="${itemInstance?.contrapunto?.id}">${itemInstance?.contrapunto?.encodeAsHTML()}</g:link></span>
